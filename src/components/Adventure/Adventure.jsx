...
export default function Adventure() {const [photos, setPhotos] = useState([]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      const response = await fetch(`${apiUrl}/api/adventure-photos`);
      if (!response.ok)
    ...
    if (isLoading) {
      ...
      return (
        <MDBContainer fluid className='adventure-gallery-container'>
          <MDBRow>
              ...
              <MDBCol lg="4" md="6" className="mb-3 pe-1" key={photo.id}>
                <div className='adventure-grid-item'>
                  <img src={photo.url} alt={`Gallery ${photo.id}`}
                       className="img-fluid adventure-gallery-image"
